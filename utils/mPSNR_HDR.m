function [mpsnr, eMax, eMin] = mPSNR_HDR(img_ref, img_dist, eMin, eMax)
%
%
...
for i=eMin:eMax
    ...
    timg_ref = quant8(img_ref, espo);
    val = mean(timg_ref(:) / 1023.0); %mean value

    if((val > 0.1) && (val < 0.9))
        eMean = [eMean, val];
        eVec  = [eVec,  i];

        timg_dist = quant8(img_dist, espo);
...
if(~isempty(eMax))
    mse = mse_acc / length(eMean);
    mpsnr = 20.0 * log10(1023.0 / sqrt(mse));
else
    mpsnr = -1;
end
...
function out = quant8(img, e)
    img_eg = (img * e);
    out = ClampImg(round(1023 * img_eg), 0.0, 1023.0);
end